...
既存利用者向けには、設定画面の「不足カテゴリ・項目を追加」ボタンで、
本テンプレートの差分のみを末尾追加できる（既存データは変更しない）。

| カテゴリ | 項目 |
|---------|------|
| コスト | 坪単価、本体工事費、付帯工事費（地盤改良・外構等）、諸経費、標準総額目安、値引き条件 |
| 設計・間取り | 自由設計の範囲、設計士の専任、打合せ回数、設計料、平屋・2階・3階対応 |
| 断熱 | 断熱工法、断熱材の種類、床断熱材、壁断熱材、天井・屋根断熱材、断熱等性能等級（UA値）、C値（気密性能） |
| 省エネ・環境 | 一次エネルギー消費量等級、ZEH対応、換気方式、太陽光発電（kW）、蓄電池 |
| 開口部（窓） | サッシの種類、ガラスの種類、玄関ドアの種類 |
| 構造 | 工法、耐震等級、制震装置、基礎の種類、地盤保証、シロアリ保証 |
| 設備（水回り） | キッチン、浴室、洗面台、トイレ、給湯器、食洗機 |
| 空調・電気 | 床暖房、全館空調、エアコン標準台数、コンセント・LAN配線 |
| 外装・内装 | 外壁材、屋根材、床材、天井高、標準収納 |
| 保証・アフターサービス | 初期保証年数、長期保証の条件、構造躯体保証、防水保証、設備保証、定期点検の頻度 |
| 会社情報 | 創業年、施工棟数、住宅完成保証、モデルハウス所在地、引渡しまでの期間 |

#### 4-11-1. 不足カテゴリ補填機能

| 項目 | 内容 |
|------|------|
| 配置 | 設定画面 → 「標準テンプレート」カード → 「📋 不足カテゴリ・項目を追加」ボタン |
| 動作 | DEFAULT_TEMPLATE と既存データを比較し、不足カテゴリ／項目のみを末尾に追加 |
| 同定ルール | カテゴリは `normalizedName`（trim + lowercase）、項目は同カテゴリ内の `name`（trim 一致） |
| 既存データ | 名前・順序・値・メモは一切変更しない（追加のみ） |
| 削除済み扱い | `deletedAt` 付きは「未存在」とみなして新規作成（旧 ID は復活させない） |
| 完了通知 | 追加件数を success Toast で表示。0件なら info Toast「追加が必要なカテゴリ・項目はありません」 |
| 確認ダイアログ | 実行前に必須（誤操作防止） |
...
### 5-1-2. デザイン要件（追加 v3.1）

UI デザインは **https://inumaru-kazuya.github.io/takomaruHP/** を参考とし、
「和モダン・暖色アースカラー」のデザイン言語を全画面で統一する。
具体的なカラーパレット・タイポグラフィ・形状ルールは `CLAUDE.md` の
「デザインガイドライン」セクションを正として実装する。

主要トークン:
- 背景: `#faf8f3` (ivory) / `#f0eadb` (paper sand)
- 罫線: `#d8cfbe` (taupe)
- 主要テキスト: `#1a1816` (warm near-black)
- アクセント: `#b35a3a` (rust)
- セカンダリ: `#c69e6c` (wood) / `#5b6b4a` (sage)
- フォント: Noto Sans JP / Noto Serif JP / Cormorant Garamond / JetBrains Mono
- ボタン形状: ピル型 (border-radius: 999px) を基本
