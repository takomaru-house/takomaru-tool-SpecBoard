...
```json
{
  ...
  "test_cases": [
    ...
    },
    {
      ...
      "expected_result": "「コスト」「設計・間取り」「断熱」「省エネ・環境」「開口部（窓）」「構造」「設備（水回り）」「空調・電気」「外装・内装」「保証・アフターサービス」「会社情報」の11カテゴリが存在すること。各カテゴリに仕様書 4-11 節記載の仕様項目（合計58項目）が存在すること。",
      "automation_hint": "IT: initializeDefaultTemplate() 後に loadCategories() で11件、loadSpecItems() で58件（または仕様書記載の各件数）が存在することを確認。"
    },
    {
      "tc_id": "TC_048b",
      "perspective_id": "F-SC-016",
      "purpose": "既存ユーザーが「不足カテゴリを追加」ボタンで DEFAULT_TEMPLATE の差分のみを末尾追加できる",
      "precondition": "旧バージョンの5カテゴリ（断熱／開口部（窓）／構造／設備（水回り）／保証・アフターサービス）のみが登録された状態",
      "steps": [
        "1. 設定画面を開く",
        "2. 「📋 不足カテゴリ・項目を追加」ボタンをクリック",
        "3. 確認ダイアログで「OK」を選択",
        "4. 仕様比較タブを開いてカテゴリ一覧を確認"
      ],
      "input_data": {
        "existing_categories": ["断熱", "開口部（窓）", "構造", "設備（水回り）", "保証・アフターサービス"]
      },
      "expected_result": "新規カテゴリ（コスト／設計・間取り／省エネ・環境／空調・電気／外装・内装／会社情報）が末尾に追加されること。既存カテゴリの並びは変わらず、不足項目（例: 断熱に C値、構造に シロアリ保証）が同カテゴリ末尾に追加されること。Toast に追加件数が表示されること。",
      "automation_hint": "IT: 5カテゴリ状態を作って fillMissingDefaultTemplate() を呼び、追加されたカテゴリ・項目を検証。重複追加が起きないこと、既存項目の sortOrder が保たれることを確認。"
    },
    {
      "tc_id": "TC_048c",
      "perspective_id": "F-SC-016",
      "purpose": "「不足カテゴリを追加」を全件揃った状態で実行しても重複追加されない（冪等性）",
      "precondition": "DEFAULT_TEMPLATE 全件が既に登録されている状態",
      "steps": [
        "1. 設定画面を開く",
        "2. 「📋 不足カテゴリ・項目を追加」ボタンをクリック → 確認 OK"
      ],
      "input_data": {},
      "expected_result": "info Toast「追加が必要なカテゴリ・項目はありません」が表示されること。カテゴリ／仕様項目の総数が変化しないこと。",
      "automation_hint": "IT: 2 回連続で fillMissingDefaultTemplate() を呼び、2 回目の addedCategories/addedItems が 0 であることを確認。"
    },
    {
      "tc_id": "TC_049",
